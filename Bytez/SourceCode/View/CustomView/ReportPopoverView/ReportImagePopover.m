...
@interface ReportImagePopover ()

@property (nonatomic, strong) UIControl *blackOverlay;
@property (nonatomic, weak) UIView *containerView;
@property (nonatomic, assign, readwrite) ReportImagePopoverPosition popoverPosition;
...
@implementation ReportImagePopover

+ (instancetype)popover
{
    return [[ReportImagePopover alloc] init];
}
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        self.sideEdge = 10.0;
        self.maskType = ReportImagePopoverMaskTypeBlack;
...
{
    NSAssert((CGRectGetWidth(contentView.bounds)>0&&CGRectGetHeight(contentView.bounds)>0), @"ReportImagePopover contentView bounds.size should not be zero");
    NSAssert((CGRectGetWidth(containerView.bounds)>0&&CGRectGetHeight(containerView.bounds)>0), @"ReportImagePopover containerView bounds.size should not be zero");
    NSAssert(CGRectGetWidth(containerView.bounds)>=CGRectGetWidth(contentView.bounds), @"ReportImagePopover containerView width should be wider than contentView width");
    ...
    switch (self.maskType) {
        case ReportImagePopoverMaskTypeBlack:
            maskColor = [UIColor colorWithWhite:0.0 alpha:0.55];
            break;
        case ReportImagePopoverMaskTypeNone:
...
- (void)showAtView:(UIView *)atView popoverPostion:(ReportImagePopoverPosition)position withContentView:(UIView *)contentView inView:(UIView *)containerView
{
    ...
    NSAssert((upCanContain||downCanContain), @"ReportImagePopover no place for the popover show, check atView frame %@ check contentView bounds %@ and containerView's bounds %@", NSStringFromCGRect(atViewFrame), NSStringFromCGRect(contentView.bounds), NSStringFromCGRect(containerView.bounds));
    
    
    CGPoint atPoint = CGPointMake(CGRectGetMidX(atViewFrame), 0);
    ReportImagePopoverPosition dxP;
    if (upCanContain) {
        dxP = ReportImagePopoverPositionUp;
        atPoint.y = CGRectGetMinY(atViewFrame) - betweenArrowAndAtView;
    }else {
        dxP = ReportImagePopoverPositionDown;
    ...
    if (upCanContain && downCanContain) {
        ...
        if (position!=0) {
            useUp = position == ReportImagePopoverPositionUp ? YES : NO;
        }
        if (useUp) {
            dxP = ReportImagePopoverPositionUp;
            atPoint.y = CGRectGetMinY(atViewFrame) - betweenArrowAndAtView;
